	...
	)
{
	...
	for (ULONG ul = 0; ul < ulPartIndexIds; ul++)
	{
		...
		if(fNLJoin)
		{
			// if it is a nestloop join with inner child being another join, and the partition consumer
			// is in the inner child, don't request dynamic partition elimination, because we may end up
			// with materializing nestloop join's inner child, which is a join contains dynamic partition
			// consumer.
			if (1 == ulChildIndex && exprhdl.Pdprel(1)->UlJoinDepth() > 1 && !fOuterPartConsumer)
			{
				pdrgpul->Release();

				// this is an invalid propagation spec, CCostContext::FValid will check validity by
				// comparing derived and required properties, and discard the plan generated by this
				// property request.
