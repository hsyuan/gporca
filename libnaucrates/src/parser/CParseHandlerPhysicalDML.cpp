	...
	)
{
	...
	const XMLCh *xmlszNeedsSort = attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenNeedsSort));
	if (NULL != xmlszNeedsSort)
	{
		m_fNeedsSort = CDXLOperatorFactory::FValueFromXmlstr
											(
											m_pphm->Pmm(),
											xmlszNeedsSort,
											EdxltokenNeedsSort,
	...
	)
{
	...
	pdxlddinfo->AddRef();
	CDXLPhysicalDML *pdxlop = GPOS_NEW(m_pmp) CDXLPhysicalDML(m_pmp, m_edxldmltype, pdxltabdesc, m_pdrgpul, m_ulAction, m_ulOid, m_ulCtid, m_ulSegmentId, m_fPreserveOids, m_ulTupleOidColId, pdxlddinfo, m_fNeedsSort);
